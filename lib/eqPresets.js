...
class EQPresetManager {
    ...
    recallEQ(channel, presetIdx) {
        // Direct mapping: UI Preset number = MIDI ID
        const id = presetIdx - 1; // Convert to 0-based MIDI ID
    ...
    processEQNames(msg) {
        ...
        console.log(`[EQ RAW] Parsing Preset ID ${id} (${id + 1})`);
        ...
        if (name.length > 0) {
            console.log(`[EQ PRESET PARSED] #${id + 1} (ID ${id}): "${name}"`);

            if (!this.state.eqPresets) this.state.eqPresets = {};
            this.state.eqPresets[id + 1] = name;
